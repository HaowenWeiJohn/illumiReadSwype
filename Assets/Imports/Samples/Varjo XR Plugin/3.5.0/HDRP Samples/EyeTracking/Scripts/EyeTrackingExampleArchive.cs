...
public enum GazeDataSourceArchive
{
    InputSubsystem,
    GazeAPI
}

public class EyeTrackingExampleArchive : MonoBehaviour
{
    [Header("Gaze data")]
    public GazeDataSourceArchive gazeDataSource = GazeDataSourceArchive.InputSubsystem;
    ...
    void Update()
    {
        ...
        if (VarjoEyeTracking.IsGazeAllowed() && VarjoEyeTracking.IsGazeCalibrated())
        {
            ...
            if (gazeDataSource == GazeDataSourceArchive.InputSubsystem)
            {
        ...
        if (Physics.SphereCast(rayOrigin, gazeRadius, direction, out hit))
        {
            ...
            KeyControllerArchive keyController = hit.collider.gameObject.GetComponent<KeyControllerArchive>();
